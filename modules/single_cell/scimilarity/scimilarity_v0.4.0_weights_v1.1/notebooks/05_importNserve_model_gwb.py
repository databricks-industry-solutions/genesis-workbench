...
run_id_gene_order = deploy_model(user_email=user_email,
                ...
                deployment_description="SCimilarity v0.4 gene-order lookup endpoint (Apache-2.0). Returns the canonical SCimilarity gene-ordering vector that callers must align their expression matrix to before requesting embeddings. CPU/Small — lightweight indexed lookup.",
...
run_id_get_embedding = deploy_model(user_email=user_email,
                ...
                deployment_description="SCimilarity v0.4 (weights v1.1) cell embedding model (Apache-2.0). Foundation model trained on ~24M cells from CellxGene; returns 128-d cell embeddings from gene-expression vectors for cross-dataset cell similarity / cell-type search.",
                input_adapter_str="none",
                output_adapter_str="none",
                sample_input_data_dict_as_json="none",
                sample_params_as_json="none",
