...
# MAGIC %md
# MAGIC # Rapids-SingleCell Analysis in a Databricks Notebook
# MAGIC  - load in the data from a **Unity Catalog Volume**
# MAGIC  - analyze the data using **rapids-singlecell** (GPU-accelerated single-cell analysis)
# MAGIC  - track results using **mlflow**
# MAGIC  - Note that filtering on pct_mt_genes and n_genes_by_counts is done first for GPU workflow to prevent CUDA out of memory errors, this differes from our non-GPU accelerated workflow.
...
## We filter on these first for GPU workflow to prevent CUDA out of memory errors
adata = adata[adata.obs["n_genes_by_counts"] < parameters["n_genes_by_counts"]]
adata = adata[adata.obs["pct_counts_mt"] < parameters["pct_counts_mt"]]

# depending on input data and requirements may which to filter
rsc.pp.filter_genes(adata, min_cells=parameters["min_cells"])
rsc.pp.filter_cells(adata, min_genes=parameters["min_genes"])
...
metrics['filter_retention'] = 100.0*adata.shape[0]/current_cells
current_cells = adata.shape[0]
...
rsc.pp.normalize_total(adata, target_sum=parameters["target_sum"])
rsc.pp.log1p(adata)

# COMMAND ----------

rsc.pp.highly_variable_genes(adata, n_top_genes=parameters['n_top_genes'], flavor="cell_ranger")
...
rsc.tl.pca(adata, n_comps=parameters["n_pcs"])
