...
echo ""
echo "▶️ [DiffDock] Validating bundle (target=$TARGET)"
echo ""

databricks bundle validate --target $TARGET $EXTRA_PARAMS

echo ""
echo "▶️ [DiffDock] Deploying bundle (target=$TARGET)"
echo ""
...
databricks bundle run --target $TARGET register_diffdock $EXTRA_PARAMS --no-wait
