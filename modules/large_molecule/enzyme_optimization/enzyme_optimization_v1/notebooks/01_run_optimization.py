...
# on the dispatcher side (UI form → start_enzyme_optimization_job) and
...
if use_inprocess_ame:
    ...
    if not torch.cuda.is_available():
        ...
        raise RuntimeError(
            ...
            "returned False. The dispatcher should route Accurate-mode "
...
try:
    with _run_ctx as run:
        ...
        # Discovery tags so the in-app "Search Past Runs"on the Guided Enzyme
        # Optimization page can find this run via the same mlflow.search_runs
        # filter pattern AlphaFold + Genomics already use:
