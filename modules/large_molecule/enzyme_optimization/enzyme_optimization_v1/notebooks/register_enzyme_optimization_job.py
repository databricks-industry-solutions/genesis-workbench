...
# MAGIC principal `CAN_MANAGE_RUN` on each — without this, the
...
USING (SELECT 'run_enzyme_optimization_job_id' AS key, '{run_enzyme_optimization_job_id}' AS value, 'large_molecule' AS module) AS source
...
spark.sql(f"""
MERGE INTO settings AS target
USING (SELECT 'run_enzyme_optimization_inprocess_ame_job_id' AS key, '{run_enzyme_optimization_inprocess_ame_job_id}' AS value, 'large_molecule' AS module) AS source
ON target.key = source.key AND target.module = source.module
WHEN MATCHED THEN UPDATE SET target.value = source.value
WHEN NOT MATCHED THEN INSERT (key, value, module) VALUES (source.key, source.value, source.module)
""")
