"""Genomics workflow routes — async-job dispatchers + MLflow result
...
from app.services import genomics as db
...
class GenomicsDefaultsResponse(BaseModel):
    variant_calling: VariantCallingDefaults
    gwas: GwasDefaults
    vcf_ingestion: VcfIngestionDefaults


@router.get("/defaults", response_model=GenomicsDefaultsResponse)
def genomics_defaults(_: CurrentUserDep) -> GenomicsDefaultsResponse:
    """Form defaults — paths to the sample datasets pre-staged by the
    `genomics/` deploy. The deploy notebooks write everything under
    deterministic per-catalog/schema paths, so we compute them here rather
    than relying on per-sample-path env vars (the form reads vars
    like `SAMPLE_FASTQ_R1` that the genomics deploy never sets,
    ...
    return GenomicsDefaultsResponse(
        variant_calling=VariantCallingDefaults(
