            ...
            <>
              ...
              <div className="rounded-md border border-border bg-card p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <h4 className="text-sm font-medium">AI interpretation</h4>
                  <button
                    type="button"
                    onClick={() => narrative.mutate()}
                    disabled={narrative.isPending}
                    className="rounded-md border border-primary/50 bg-primary/10 px-3 py-1.5 text-xs text-primary hover:bg-primary/20 disabled:opacity-50"
                  >
                    {narrative.isPending
                      ? 'Generating…'
                      : narrative.data
                        ? 'Regenerate'
                        : 'Interpret with Claude (Opus 4.8)'}
                  </button>
                </div>
                {narrative.isError && (
                  <p className="text-xs text-destructive">{String(narrative.error)}</p>
                )}
                {narrative.data ? (
                  <>
                    <Markdown>{narrative.data.narrative}</Markdown>
                    <p className="mt-2 text-[10px] text-muted-foreground">
                      Generated by {narrative.data.model}. AI-generated — verify against the data.
                    </p>
                  </>
                ) : (
                  !narrative.isPending && (
                    <p className="text-xs text-muted-foreground">
                      Generate a plain-language interpretation of this result — which genes moved,
                      what program they belong to, and the caveats — grounded only in the data above.
                    </p>
                  )
                )}
              </div>
