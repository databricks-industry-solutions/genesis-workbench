...
echo ""
echo "▶️ [Protein_MPNN] Validating bundle (target=$TARGET)"
echo ""

databricks bundle validate --target $TARGET $EXTRA_PARAMS

echo ""
echo "▶️ [Protein_MPNN] Deploying bundle (target=$TARGET)"
echo ""
...
echo ""
databricks bundle run --target $TARGET register_proteinmpnn $EXTRA_PARAMS --no-wait
