...
echo ""
echo "▶️ [RFdiffusion] Validating bundle (target=$TARGET)"
echo ""

databricks bundle validate --target $TARGET $EXTRA_PARAMS

echo ""
echo "▶️ [RFdiffusion] Deploying bundle (target=$TARGET)"
echo ""
...
echo ""
databricks bundle run --target $TARGET register_rfdiffusion $EXTRA_PARAMS --no-wait
