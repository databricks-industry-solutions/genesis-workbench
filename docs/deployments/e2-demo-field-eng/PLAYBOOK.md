...
**Runtime errors you may hit (and how to recover):**

| Error / symptom | What it actually means | Workaround |
|---|---|---|
| **`Annotation failed: Embedding failed for cells 0-5: NaN detected in embeddings`** (Single Cell → Cell Type Annotation) | The h5ad input wasn't preprocessed: scGPT/SCimilarity expect log-normalized counts; raw counts or files with all-zero genes / Inf values produce NaN at the embedding step. | (1) Run the same h5ad through Single Cell → **Processing** (Scanpy or RAPIDS) first, then point Annotation at the *processed* output. (2) Test with the canonical sample `/Volumes/mmt_gwb/genesis_workbench/raw_h5ad/0ae6f031-2f9c-4247-8b26-db320d6efd32.h5ad` to isolate input vs pipeline issue. (3) If still failing, check the input for genes outside the model's training vocabulary. |
| **App hangs at "Initializing…" or `home_user_settings` spinner** | The app's first SQL query against the warehouse is timing out — usually because the warehouse is cold or its launch is failing. | Check warehouse state: `databricks warehouses get <id>`. If `STARTING / FAILED` with `SECRET_CREATION_FAILURE`, that's a **platform-side serverless cluster-launch failure** (Databricks infra, not your config). Either wait for auto-retry (1-3 min) or temporarily switch the warehouse to **Pro Classic** (no serverless secret-creation step). For demo prep: bump warehouse `auto_stop_mins` from the default 10 → 60+ so it doesn't fall asleep during a long warmup window. |
| **`Predicting structure with Boltz...` hangs forever; endpoint is READY** | Endpoint container started but the Boltz model layer disabled itself due to PyTorch version mismatch (`PyTorch >= 2.4 required but found 2.3.1+cu121`). | **DO NOT click Boltz live.** Use ESMFold for live structure-prediction demo. Show Boltz value prop conceptually + via Past Runs only. Real fix: re-register Boltz with `torch>=2.4` pinned in `pip_requirements`. |
| **BioNeMo / Parabricks job: `pip install ... exit code 1`** | Custom Docker images can't reach public PyPI (default Databricks pip proxy isn't injected into custom containers). | Patch the cluster's `spark_env_vars` to set `PIP_INDEX_URL=https://pypi-proxy.dev.databricks.com/simple` (Databricks-internal proxy). Affects `bionemo_esm_finetune_job`, `bionemo_esm_inference_job`, and `parabricks_cluster`. |
| **Scanpy: `FileNotFoundError: ensembl_genes_hsapiens.csv` (or mmusculus)** | The reference gene-mapping CSV is in the `rapids_reference/` volume but Scanpy reads from `scanpy_reference/`. | `databricks fs cp` the file from rapids_reference to scanpy_reference. Pre-copy the mmusculus version too while you're there. |
| **GWAS: `No significant results found for this run. All p-values may be NULL`** | Phenotype doesn't correlate with any variants in the supplied VCF region (e.g., chr6 VCF + breast-cancer phenotype = empty plot, since BRCA hits are on chr17/13). | Either pre-engineer a phenotype that targets your VCF's chromosome (see `scripts/generate_chr6_immune_phenotype.py` for the chr6 HLA example) or fall back to a pre-built Manhattan plot screenshot for the visual. |
