...
- **1000 Genomes downloads come from the AWS Open Data S3 mirror** (`https://1000genomes.s3.amazonaws.com/...`), not EBI's `ftp.1000genomes.ebi.ac.uk` HTTPS, which is too slow from the workshop and times out (30–60 min) on multi-GB files. If you add/adjust a genomics download, keep it on S3 — the reference FASTA + `.fai` + BWA index, the HG00096 FASTQs, and the sample chr6 VCF (Phase 3 GRCh38-lifted: `release/20130502/supporting/GRCh38_positions/ALL.chr6.phase3_shapeit2_mvncall_integrated_v3plus_nounphased.rsID.genotypes.GRCh38_dbSNP_no_SVs.vcf.gz`) are all mirrored. To locate any object, list it: `https://1000genomes.s3.amazonaws.com/?list-type=2&prefix=<path>`.
